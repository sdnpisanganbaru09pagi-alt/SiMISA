...
/* Photo inputs */
async function handlePhotoChange(e, previewId) {
  ...
  const photoId = await processPhotoInput(selectedFile, el(previewId));
...
if(el('borrowPhoto')) el('borrowPhoto').addEventListener('change', async e => {
  await handlePhotoChange(e, 'borrowPreview');
...
if(el('returnPhoto')) el('returnPhoto').addEventListener('change', async e => {
  await handlePhotoChange(e, 'returnPreview');
